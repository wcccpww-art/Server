...
def main():
    ...
    s.add_argument("-l", "--list-groups", action="store_true",
                   dest="list_groups", help="list all available groups/channels for the user")
    s.add_argument("--import-zip", action="store", type=str,
                   dest="import_zip", help="import data from a ZIP file containing Telegram export")
    s.add_argument("--import-downloads", action="store_true",
                   dest="import_downloads", help="import all files from downloads directory")
    ...
    # Import from ZIP.
        ...
        cfg = get_config(args.config)
        ie = ImportExport(cfg, DB(args.data))
        ie.import_zip(args.import_zip)

    # Import from downloads.
    elif args.import_downloads:
        from .import_export import ImportExport

        cfg = get_config(args.config)
        ie = ImportExport(cfg, DB(args.data))
        ie.import_downloads('downloads')
