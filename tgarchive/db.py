...
Message = namedtuple(
    "Message", ["id", "type", "date", "edit_date", "content", "reply_to", "user", "media", "category"])
...
class DB:
    ...
    def insert_message(self, m: Message):
        ...
        cur.execute("""INSERT OR REPLACE INTO messages
            (id, type, date, edit_date, content, reply_to, user_id, media_id, category)
            VALUES(?, ?, ?, ?, ?, ?, ?, ?, ?)""",
                    (m.id,
                     m.type,
    ...
    def _make_message(self, m) -> Message:
        ...
        id, typ, date, edit_date, content, reply_to, \
            ...
            media_id, media_type, media_url, media_title, media_description, media_thumb, category = m
